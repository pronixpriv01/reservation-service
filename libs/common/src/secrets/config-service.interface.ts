...
@Injectable()
export class ConfigServiceInterface extends ConfigService {
  private readonly logger = new Logger(ConfigServiceInterface.name);
  getSecretOrEnvValue(key: string): string {
    ...
    try {
      ...
      if (value) {
        ...
        this.logger.warn(`'${key}' not found as Docker secret or environment variable`);
      }
    }
  }
}